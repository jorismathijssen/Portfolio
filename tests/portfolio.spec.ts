...
test.describe("Portfolio E2E", () => {
  test("boot sequence and main content load", async ({ page }) => {
    ...
    await expect(page.locator("#hero").getByText("Senior Software Engineer")).toBeVisible({
      timeout: 10000,
    });
  });
  ...
  test("toggle api view", async ({ page }) => {
    ...
    await expect(page.getByText('"x-powered-by"')).toBeVisible();
    await expect(page.getByText('".NET 8 (Simulated)"')).toBeVisible();
